...
An SQL query optimizer can be written in many ways. Its optimization also depends on how the data is stored in the file organization. 
5. **Query Execution** – DBMS **fetches data from tables** (using indexes if possible).
...
6. **Result Processing** – Data is formatted and sent **back to the client**.
